...
public class AppController {
	...
	public String QueryPage(@RequestParam(value = "PageSize")int PageSize,@RequestParam(value="PageNum") int PageNum,Model model){


		int count = appInfoService.count();
		PageSize = Constants.pageSize;
		int TotalPageNum = (int) Math.floor( count /PageSize);
		if (PageNum < 1){
			PageNum = 1;
		...
		List<Appinfo> list = appInfoService.QueryPage(PageSize,PageNum);

		model.addAttribute("AppList",list);
		model.addAttribute("TotalPageSize",TotalPageNum);
